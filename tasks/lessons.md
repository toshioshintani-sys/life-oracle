...
## 2026-07-07 【運用】Slack報告が事実上機能しておらず、日次/週次ローカルタスクの停止を依頼された

**状況**：俊雄さんより「Slack報告が事実上停止しているので、毎日のルーチンを全て停止する」と依頼。対象は主に姉妹プロジェクト（サブスクやめた）のpush後Slack通知運用ルールと、ライフオラクル側のローカルスケジュールタスク。

**確認事項**：このセッションはクラウドのリモート実行環境で動いており、ローカルWindows PC上のタスクスケジューラ（`LifeOracle_RunwayCheck_Weekly` 毎週日曜18:00・`LifeOracle_TruthSourceCheck_Daily` 毎日07:00）やClaude Code CLIのローカル`scheduled-tasks`（サブスクやめた側の朝会など）には**アクセス・停止操作ができない**（別マシン上のプロセス）。

**対処**：
- `CLAUDE.md`「自律運用ルール」に停止依頼の注記と、俊雄さんがご自身のPCで実行する `Disable-ScheduledTask` コマンドを追記。
- サブスクやめた側は `subsuku-yameta-web/CLAUDE.md` §11（push後Slack通知）に停止バナーを追記・§12改訂履歴に記録。

**再発防止**：
- クラウドのリモートセッションから「ローカルPCの自動化を止めてほしい」と依頼された時は、**実行できると勘違いせず**、対象がローカルタスクスケジューラかクラウド側のRoutineかをまず切り分ける。ローカル側は必ず俊雄さんご自身の操作が必要（正確なコマンドを提示するところまでが役割）。
- 「通知ルールを文書に書いた」＝「実際に届いている」ではない。運用ルールの健全性は俊雄さんの受信実績で検証する。
