...
### 2026-09-24（結末）PC再起動でPowerShellの不調が解消・X投稿は完全復旧

- **PowerShellの起動不調は PC 再起動で解消した**（`powershell -Command "1"` が60秒超 → **2.5秒**）。原因は特定していない。同じ症状（PowerShell系だけ遅く、`schtasks` 等は即応答）が出たら、**まず再起動を提案するのが最短**。
- 再起動後、ログオン時トリガーで常駐が**正しく起動**した（PID 8352・ロックファイルも一致・1プロセスのみ）。「常駐起動がPowerShell経由なので再起動後に立ち上がらないかも」という懸念は杞憂だった。
- **22:30枠で実投稿を確認**（X上で 22:30:48 JST の新規ポストを目視）。15日ぶりの自動投稿。
- ⚠️ 残る不具合：posts_log に記録される**投稿IDが実物と違う**（今回も記録は 2088385029152109055＝8/15の投稿ID、実物は 2090431021665972288）。IDを頼りに投稿の存在確認をしないこと。**存在確認はタイムラインを見る。**
