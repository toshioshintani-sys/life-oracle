            ...
            <div className="result-biases-mini" style={{ marginTop: 20 }}>
              {biases.map((b, i) => (
                <div key={i} className={`result-bias-mini bias-rank-${i + 1}`}>
                  ...
                  {(b.magazineUrl || (b.noteUrl && new Date() >= new Date(b.noteScheduledAt ?? 0))) && (
                    <a href={b.magazineUrl ?? b.noteUrl} target="_blank" rel="noopener noreferrer" className="bias-note-link-mini">詳しく →</a>
                  )}
                </div>
              ))}
            </div>
