...
function buildDeityPrompt({ generation, occupation, killerLine, praiseText, habitText, mechanismWhy, biasName, biasDesc, prescriptionText }) {
  ...
  const lines = [
    ...
    '',
    // 2026-08-07: 性別が職業から推測される不具合を修正。
    // 20代・理美容師で実行すると（男性でも）女性として描かれた。原因はプロンプト内の矛盾で、
    // 「確信が持てない場合のみ確認して」と書きつつ末尾で「文章は書かず画像のみ」と命じていたため
    // 確認する道が塞がれ、AIは必ず推測するしかなく、職業の連想（理美容師＝女性）で埋まっていた。
    // 「聞く」を諦めて「書ける欄」に置き換える。空欄でも従来どおり推定は走る。
    '【手順1：私の性別（画像に反映するだけ・文章では説明しない）】',
    '私の性別：',
    'これまでの私との会話やメモリから私の性別を推定し、画像に反映して。推定の説明文は書かないこと。',
    attrs ? `私の固定属性：${attrs}。` : '',
    '',
            ...
            {top2.map((biasId, idx) => {
              ...
              return (
                <div key={biasId} className={`mbti-bias-card${idx === 0 ? ' mbti-bias-card--top' : ''}`}>
                  ...
                  {info?.talkLine && (
                    <p className="mbti-bias-talk">
                      <span className="mbti-bias-talk-label">人に話すなら</span>
                      {info.talkLine}
                    </p>
                  )}
