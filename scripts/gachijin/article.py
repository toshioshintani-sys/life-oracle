...
- カーリー引用符の代わりに「」を使う
- 仕上げ基準（AI感ゼロ化）: 文の長さに変化をつけてリズムを作ること。テンプレ的な言い回し・予測可能な表現・説明口調を避け、実際にその状況を経験した人が書いたような自然な温度感にすること。ただし温度感は心理ワードや感情語（孤独・不安・疲れ等）を足すことでは出さない。上記moment欄と同じく、具体的な場面・仕草・言いよどみで温度を出し、答えを説明せず読者が自分で気づく余白を残すこと。
