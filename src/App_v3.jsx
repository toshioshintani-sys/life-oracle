...
import { calcScore, getTypeName, getTendencyLabel, calcBiasScores, biasInfo } from "./utils/scoring.js";
...
// MBTI → 旧Jung typeID マッピング（bias_messages.json キー用）
const MBTI_TO_JUNG = {
  ESTJ: 'Te-光', ENTJ: 'Te-影',
  ESFJ: 'Fe-光', ENFJ: 'Fe-影',
  ESTP: 'Se-光', ESFP: 'Se-影',
...
// ─── プロンプト生成 ──────────────────────────────────────
function generateSystemPrompt(mbtiType, axisScores, biasTop2, typeProfile, occupationLabel, generationLabel) {
...
function generateCopyPrompt(mbtiType, axisScores, biasTop2, typeProfile, occupationLabel, generationLabel) {
  const praise = typeProfile?.praiseText ?? "";
  const habit = typeProfile?.habitText ?? "";
  const axes = [
    `EI: ${axisScores.EI}点（${axisScores.EI >= 13 ? 'E寄り' : 'I寄り'}）`,
...
export default function App() {
  ...
  // バイアスメッセージキー（旧形式: {Jung typeId}_{biasMessageKey}）
  ...
  // チャットコンテナ内を末尾へスクロール（ページ全体はスクロールしない）
  useEffect(() => {
  ...
  function handleCopyPrompt() {
    ...
    const prompt = generateCopyPrompt(mbtiType, scoreResult.scores, top2, typeProfile, occupationLabel, generationLabel);
  ...
  function handleShareCopy() {
    ...
    const text = [
      ...
      top2.length >= 2
        ? `思考のクセ：${biasInfo[top2[0]]?.name} / ${biasInfo[top2[1]]?.name}`
        : '',
      '',
      '🔮 ライフオラクルで診断する',
  ...
  async function handleChatSend() {
    ...
    const systemPrompt = generateSystemPrompt(mbtiType, scoreResult.scores, top2, typeProfile, occupationLabel, generationLabel);
    ...
    try {
      ...
      if (!res.ok) {
        setChatError(data?.error ?? "エラーが発生しました");
      } else {
      ...
      <div style={{ maxWidth: 560, margin: "0 auto" }}>
        ...
        {phase === "intro" && (
          <div style={CARD_STYLE}>
            ...
            <button onClick={() => setPhase("occupation")} style={{ width: "100%", padding: 16, background: "linear-gradient(135deg, #2d1b69, #1a0a2e)", border: `1px solid ${ACCENT}`, borderRadius: 12, color: TEXT, fontSize: 14, letterSpacing: 2, cursor: "pointer" }}>
              診断を始める
            </button>
          </div>
        )}
        ...
        {(phase === "jung" || phase === "bias") && currentQuestion && (
          <div style={{ ...CARD_STYLE, opacity: animating ? 0.7 : 1, transition: "opacity 0.2s" }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
              ...
              {isBiasPhase && (
                <div style={{ fontSize: 11, color: ACCENT, letterSpacing: 1 }}>あなたのクセを調べます</div>
              )}
            </div>
            ...
            {isJungPhase && (
              <div style={{ fontSize: 12, color: TEXT_MUTED, marginBottom: 14 }}>テーマ：{currentQuestion.tag}</div>
            )}
          ...
          <>
                ...
                return (
                  <div key={biasId} style={{
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
                      <span style={{ fontSize: 11, color: index === 0 ? ACCENT : TEXT_MUTED, letterSpacing: 1 }}>
                        {index + 1}位のクセ
                      </span>
            ...
            <div style={CARD_STYLE}>
              ...
              <p style={{ fontSize: 12, color: TEXT_MUTED, marginBottom: 16 }}>
                あなたの診断結果を踏まえたAIアシスタントに、仕事・キャリア・人間関係など何でも相談できます。
              </p>
              ...
              ref={chatContainerRef}>
                {chatMessages.length === 0 && (
                  <div style={{ fontSize: 13, color: TEXT_MUTED, textAlign: "center", padding: "24px 0" }}>
                    気になることを入力してください
                  </div>
                )}
                ...
                {chatError && (
                  <div style={{
                    ...
                    border: "1px solid rgba(216,100,100,0.3)",
                    fontSize: 13,
                    color: "#d86464",
                  }}>
                    {chatError}
              ...
              <div style={{ display: "flex", gap: 8 }}>
                ...
                <button
                  ...
                  style={{
                    ...
                    borderRadius: 10,
                    color: (!chatInput.trim() || chatLoading) ? TEXT_MUTED : TEXT,
                    fontSize: 14,
            ...
            <button onClick={handleReset}
              style={{ width: "100%", padding: 14, background: "transparent", border: `1px solid rgba(196,148,10,0.3)`, borderRadius: 10, color: TEXT_MUTED, fontSize: 14, cursor: "pointer", marginBottom: 20 }}>
              もう一度診断する
            </button>
          </>
        )}
